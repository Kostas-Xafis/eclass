...
$(document).ready(function () {
	...
	$("#module_toggle").on("click", function (e) {
		...
		$.post(url, { hide: $(this).data("state") }, function () {
			location.reload();
		});
	});
	...
	$(".extapp-status").on("click", function () {
		...
		$.post(url, { state: state, appName: appName }, function (data) {
	...
	if ($(".collapse.in").length > 0) {
		//when page first loads the show.bs.collapse event is not triggered
