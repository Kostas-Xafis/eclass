...
if (isset($_POST['submit'])) {
    ...
    if (!$fac) { // if user does not belong to department
        ...
        $tool_content .= '
        <form style="margin-bottom:2rem; width: 50%">
            <div class="input-group">
                <label class="sr-only" for="search_courses">Αναζήτηση</label>
                <input type="text" class="form-control input-sm" id="search_courses" name="search_courses" placeholder="Αναζήτηση Μαθήματος ή ανά καθηγητή...">
        ...
        <script>
            let input = document.getElementById("search_courses");
            let rows = null
            let rowsText = null;
            input.addEventListener("input", function() {
                if (rows === null) {
                    rows = [...document.querySelectorAll("#main-content > div > div > div.table-responsive > table tr")].slice(1)
                    rowsText = rows.map(row => row.textContent.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
                }
                let value = input.value.toLowerCase();
                rows.forEach((row, index) => {
                    row.classList.toggle("hide", !rowsText[index].includes(value));
                });
            });
        </script>
';
