...
if (isset($_POST['submit'])) {
    ...
    while ($line !== false) {
        ...
        if (!empty($line)) {
            $info = explode('|', $line);
            $title = $info[0];
            $course_code = isset($info[1]) ? $info[1] : '';
            $prof_uid = null;
            $prof_not_found = false;
            if (isset($info[2])) {
                $prof_info = trim($info[2]);
                $prof_uid = find_prof(trim($info[2]));
            ...
            list($code, $cid) = create_course($course_code, $_POST['lang'], $title, '', $departments, $vis, $prof_name, $_POST['password']);
            if ($cid) {
                if ($prof_uid) {
...
function add_prof($uid, $name, $description)
{
    $names = explode(' ', $name);
    if (count($names) == 2) {
        $givenname = $names[0];
    ...
    Database::get()->query("INSERT INTO user
                            SET username = ?s,
                                ...
                                expires_at = " . DBHelper::timeAfter(strtotime('+1 year')) . ",
                                lang = ?s",
                            $uid, // Assuming $uid is used as username
                            password_hash(generate_password(), PASSWORD_DEFAULT),
                            $givenname,
                            $surname,
                            '', // Assuming email is empty
                            $description,
                            'en'); // Assuming default language is English
