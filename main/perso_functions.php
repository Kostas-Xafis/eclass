    ...
    if ($myCourses) {
        ...
        foreach ($myCourses as $data) {
            ...
            $lesson_content .= "<tr class='$visclass'>
			  <td class='text-left'>
			  <strong><a href='${urlServer}modules/document/?course=$data->code'>" . q(ellipsize($data->title, 64)) . "</a></strong>&nbsp;(" . q($data->public_code) . ")
        ...
        // The link should be clicked even when the whole tr is clicked
        $lesson_content .= "<script>
            $('#portfolio_lessons tr').click(function() {
