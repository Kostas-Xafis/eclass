...
function lessonToolsMenu($rich=true) {
    ...
    foreach ($tools_sections as $section) {
        ...
        if ($section['title'] == $GLOBALS['langCourseOptions'] || $section['title'] == $GLOBALS['langActiveTools']) {
            // Update "Αρχική" menu item with new URL and icon
            array_push($sideMenuText, 'Αρχική');
            array_push($sideMenuLink, '/courses/' . $course_code.'/');
            array_push($sideMenuImg, 'fa-university');
        }
